<section class="we-suggest cslider py-20 pb-20 overflow-hidden" id="suggest">
        ...
        <div class="therapies-slider -mx-4">
            @foreach ($services as $service)
                <div class="therapy-col">
                    <a href="{{ route('service', $service) }}/kz" class="h-full">
                        <div class="mx-4-2 flex h-full items-center justify-center rounded-lg" style="background-image: url({{ Voyager::image($service->thumb) }})">
                            ...
                            <div class="z-10 text-center text-base lg:text-xl text-white w-3/4">{{ $service->title_kz }}</div>
                        </div>
                    </a>
                </div>
            @endforeach
