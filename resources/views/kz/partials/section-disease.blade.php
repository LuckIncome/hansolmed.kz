<section class="useful cslider py-20 pb-20 @if($bg == 'dark') bg-primary-dark @endif overflow-hidden" id="disease">
        ...
        <div class="therapies-slider -mx-4">
            @foreach ($ailments as $ailment)
                <div class="therapy-col">
                    <a href="{{ route('ailment', $ailment) }}/kz" class="h-full">
                        <div class="mx-4-2 flex h-full items-center justify-center rounded-lg" style="background-image: url({{ Voyager::image($ailment->thumbnail('cropped', 'thumb')) }})">
                            ...
                            <div class="z-10 text-center text-base lg:text-xl text-white w-3/4">{{ $ailment->title_kz }}</div>
                        </div>
                    </a>
                </div>
            @endforeach
